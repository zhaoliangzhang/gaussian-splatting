...
def measure_fps(scene, gaussians, pipeline, background):
    with torch.no_grad():
        ...
        t0 = benchmark.Timer(stmt='render_fn(views, gaussians, pipeline, background)',
                            setup='from render import render_fn',
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, prune, skip_train : bool, skip_test : bool):
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), pr.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test)